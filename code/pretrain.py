...
def parse_args():
    ...
    # Conventional args
    parser.add_argument('--pre_data_dir', type=str, default=os.environ.get('SM_CHANNEL_TRAIN', '../input/data/KAIST_MIX_824'))
    parser.add_argument('--fine_data_dir', type=str, default=os.environ.get('SM_CHANNEL_TRAIN', '../input/data/ICDAR17_Korean'))
    ...
    parser.add_argument('--pre_epoch', type=int, default=300)
    parser.add_argument('--fine_epoch', type=int, default=300)
...
def do_training(pre_data_dir, fine_data_dir, model_dir, device, image_size, input_size, num_workers, batch_size,
                learning_rate, pre_epoch, fine_epoch, save_interval, log_wandb, project_name, exp_name, log_val):

    pre_dataset = SceneTextDataset(pre_data_dir, split='train', image_size=image_size, crop_size=input_size)
    pre_dataset = EASTDataset(pre_dataset)
    pre_num_batches = math.ceil(len(pre_dataset) / batch_size)
    pre_train_loader = DataLoader(pre_dataset, batch_size=batch_size, shuffle=True, num_workers=num_workers)

    fine_dataset = SceneTextDataset(fine_data_dir, split='train', image_size=image_size, crop_size=input_size)
    fine_dataset = EASTDataset(fine_dataset)
    fine_num_batches = math.ceil(len(fine_dataset) / batch_size)
    fine_train_loader = DataLoader(fine_dataset, batch_size=batch_size, shuffle=True, num_workers=num_workers)

    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    model = EAST()
    model.to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate)
    scheduler = lr_scheduler.MultiStepLR(optimizer, milestones=[ (pre_epoch+fine_epoch) // 2], gamma=0.1)

    model.train()

    ## pretrain epoch
    print(f"{pre_epoch}에폭 Pretrain을 시작합니다")
    for epoch in range(pre_epoch):
        epoch_loss, epoch_start = 0, time.time()
        epoch_loss_cls, epoch_loss_angle, epoch_loss_iou = 0, 0, 0
        
        with tqdm(total=pre_num_batches) as pbar:
            for img, gt_score_map, gt_geo_map, roi_mask in pre_train_loader:
        ...
        # 에폭당 평균 로스들을 구함
        mean_loss = epoch_loss / pre_num_batches
        mean_loss_cls = epoch_loss_cls/pre_num_batches
        mean_loss_angle = epoch_loss_angle/pre_num_batches
        mean_loss_iou = epoch_loss_iou/pre_num_batches
        ...
        print('Mean loss: {:.4f} | Elapsed time: {}'.format(
            epoch_loss / pre_num_batches, timedelta(seconds=time.time() - epoch_start)))
    ...
    ## fine-tune epoch
    print(f"{fine_epoch}에폭 Fine-tuning을 시작합니다")
    for epoch in range(fine_epoch):
        epoch_loss, epoch_start = 0, time.time()
        epoch_loss_cls, epoch_loss_angle, epoch_loss_iou = 0, 0, 0
        
        with tqdm(total=fine_num_batches) as pbar:
            for img, gt_score_map, gt_geo_map, roi_mask in fine_train_loader:
                pbar.set_description('[Epoch {}]'.format(epoch + 1))
        ...
        # 에폭당 평균 로스들을 구함
        mean_loss = epoch_loss / fine_num_batches
        mean_loss_cls = epoch_loss_cls / fine_num_batches
        mean_loss_angle = epoch_loss_angle / fine_num_batches
        mean_loss_iou = epoch_loss_iou / fine_num_batches
        ...
        print('Mean loss: {:.4f} | Elapsed time: {}'.format(
            epoch_loss / fine_num_batches, timedelta(seconds=time.time() - epoch_start)))
        print(f'Mean cls loss: {mean_loss_cls:.4f} | Mean angle loss: {mean_loss_angle:.4f} | Mean iou loss: {mean_loss_iou:.4f}')
...
def main(args):
    ...
    if args.log_val == "True":
        print("prtrain은 validation을 지원하지 않습니다")
