...
def do_training(data_dir, model_dir, device, image_size, input_size, num_workers, batch_size,
    ...
    model = EAST()
    model.to(device)
    optimizer = torch.optim.AdamW(model.parameters(), lr=learning_rate) # optimizer 수정
    scheduler = lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=15, eta_min=1e-6, verbose=True) # scheduler 수정
    ...
    for epoch in range(max_epoch):
        ...
        # save_interval 마다 pth 저장하기 위한 코드
        if (epoch + 1) % save_interval == 0:            
            ckpt_fpath_for_backup = osp.join(model_dir, f'{exp_name}_{epoch+1}.pth')
            torch.save(model.state_dict(), ckpt_fpath_for_backup)
