...
class SceneTextDataset2(Dataset):
    def __init__(self, root_dir, split='train', image_size=1024, crop_size=512, color_jitter=True,
                 normalize=True):
        with open(osp.join(root_dir, 'ufo/splited_{}.json'.format(split)), 'r') as f:
            anno = json.load(f)
        ...
        ## validset일 경우 transform을 안하기 위해서 만든 tag
        self.transform_tag = True

        if split == 'valid':
            self.transform_tag = False
